...
contract SimpleCrowdfund {
    ...
    uint256 public constant minimalAmount = 1e15; // ~3$
    ...
    uint256 public immutable i_secToComplete = 30; //3600 * 24;
    ...
    mapping(address contributor => uint256 amountDonated) private s_contributorToAmount;
    ...
    function contribute() public payable isWithdrawned isGoalReached{
        ...
        if (msg.value < minimalAmount) revert SimpleCrowdfund__ToLittleDonation();
        if (amountRaised >= GOAL) goalReached = true;
        if (timePassed()) revert SimpleCrowdfund__TimeOut();
    ...
    function refund() public isWithdrawned{// !!!! Add refund for every backer seperately
        ...
        if (timePassed() && amountRaised < GOAL) {
            ...
            else revert SimpleCrowdfund__NoPermission();
            // for (uint256 ContributorIndex = 0; ContributorIndex < ContributorsList.length; ContributorIndex++) {
            //     (bool callSuccess,) = payable(ContributorsList[ContributorIndex]).call{value: address(this).balance}("");
            //     if (!callSuccess) revert SimpleCrowdfund__CallFailed();
            //     else emit Withdraw(i_owner, amountRaised);
